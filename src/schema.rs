...
impl FnSchema {
    ...
    pub fn schema(&self) -> Value {
        ...
        for (name, required, s) in (self.params)(&mut g) {
            ...
            if !doc.is_empty() { v.as_object_mut().map(|o| o.insert("description".into(), doc.into())); }
            props.insert(name.into(), v);
            if required { req.push(Value::from(name)) }
        }
        let mut input = json!({"type": "object", "properties": props});
        if !req.is_empty() { input["required"] = req.into(); }
        let defs = g.take_definitions(false);
        if !defs.is_empty() { input["$defs"] = Value::Object(defs); }
...
pub fn get_schema(name: &str) -> Option<Value> {
    ...
    if let Some(f) = all.iter().find(|f| f.docs.path() == name) { return Some(f.schema()); }
    let suffix = format!("::{name}");
    let mut it = all.iter().filter(|f| f.docs.name == name || f.docs.path().ends_with(&suffix));
    match (it.next(), it.next()) { (Some(f), None) => Some(f.schema()), _ => None }
}
